...
test('production analytics wires bounded events and canonical URLs while exports stay local', async ({
  ...
  await expect
    ...
    .toBe(true);
  await page.goto('https://www.chartsai.com/guides/histogram-bin-width/?query=SECRET');
  const guideDownload = page.waitForEvent('download');
  await page.locator('.comparison-downloads').first().getByRole('link', { name: 'CSV', exact: true }).click();
  await guideDownload;
  await expect
    .poll(() =>
      received.some(
        (event) =>
          event.n === 'Chart Download' && event.p.guide === 'histogram-bin-width' && event.p.format === 'csv',
      ),
    )
    .toBe(true);
  await page
    ...
    .first()
    .getByRole('link', { name: 'Edit this view →', exact: true })
    .click();
  await expect
    .poll(() =>
      received.some((event) => event.n === 'Example Loaded' && event.p.guide === 'histogram-bin-width'),
    )
    .toBe(true);
...
test('template selections and subsequent downloads retain only a fixed template ID', async ({
  page,
  request,
}) => {
  const received: Record<string, any>[] = [];
  await page.route('https://www.chartsai.com/**', async (route) => {
    ...
    await route.fulfill({ response: await request.get(`http://127.0.0.1:4321${u.pathname}`) });
  });
  await page.route('https://plausible.io/js/**', (route) =>
    route.fulfill({
      contentType: 'text/javascript',
  ...
  await page.locator('#commute-gap').getByRole('link', { name: 'SVG', exact: true }).click();
  await svg;
  await expect
    .poll(() =>
      received.some(
        (e) => e.n === 'Chart Download' && e.p.template === 'commute-gap' && e.p.format === 'svg',
      ),
    )
    .toBe(true);
  await page.locator('#commute-gap').getByRole('link', { name: 'Use this chart', exact: true }).click();
  await expect(page.locator('[data-chart-ready="true"]')).toBeVisible();
  ...
  await page.getByLabel('Publication CSV data').fill('PRIVATE category,Before,After\nSECRET row,1,2');
  ...
  await expect
    .poll(() =>
      received.some(
        (e) => e.n === 'Chart Download' && e.p.template === 'commute-gap' && e.p.format === 'json',
      ),
    )
    .toBe(true);
