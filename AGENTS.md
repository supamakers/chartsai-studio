...
## Editorial publishing collection — v0.11.0

The user approved the free newsroom/publication collection on September 9, 2026. Search evidence and limitations are in `docs/EDITORIAL-RESEARCH-2026-09-09.md`. This is an extension of the existing workbenches and chart path; the homepage retains its three primary paths. Three new makers (dumbbell, slopegraph and small multiples), an editorial hub, two sourced worked resources and a publishing guide bring the intended inventory to 117 site HTML pages / 116 sitemap routes. Five additional downloadable HTML documents are assets, excluded from the sitemap and served noindex on ChartsAI; they are reusable standalone publication files.

`src/lib/editorial.ts` owns strict version-1 SupaMakers chart projects, annotations, native ECharts options, HTML composition and the AnimCharts dated-line adapter. `EditorialEditor.tsx` reuses `ChartJourney` and `ImportData`. Existing line and bar makers retain their standard flow and can open publication mode with the applied data; returning restores the original standard chart. Public query `publication=1` loads a fixed sourced example, never private query data. No accounts, model API, watermark, payment or automatic transmission is added.

Annotations reference exact row labels and series names, never screen pixels. Replacing a table explicitly clears its annotations and unchanged sample attribution; restoring a project restores all validated metadata. Invalid imports preserve the chart; unapplied CSV disables exports. Shared small-multiple scales and numeric/date X spacing are required. Bar charts include zero. Two-column paired comparisons do not imply the intermediate path, percent change or causality. Check dense endpoint labels at publication size; enlarging the preview is available.

Pinned NASA GISTEMP and World Bank WDI responses, selected CSVs and a checksum manifest are under `public/editorial/data/`. Re-fetching is explicit, not part of builds. Do not present these worked historical periods as a live news feed or latest available data. Keep World Bank CC BY 4.0 attribution and NASA's source/scientific citation; do not imply source endorsement. `src/data/editorial-data.json` must match the retained sources, verified by tests.

Exports include SVG, PNG, raster-in-PDF, CSV, editable JSON and script-free self-contained HTML with a data table and source link. All use the shared native options. Generated `public/editorial/assets/` stays out of Git/source ZIP and is rebuilt. The original project is distinct from an ECharts option file.

The sibling `../animstats` is owned by another active session. This release only reads its current `editorial-line` v1 contract and validates our adapter against it; do not modify its files. The adapter accepts dated line projects up to 150 rows / three series and reports incompatible limits instead of discarding values. Palette/layout adapt to AnimCharts; the original ChartsAI project is retained as extra JSON metadata. Transfer is a user-downloaded file, never a URL or background upload. Do not claim that other chart families animate or that every future AnimCharts version remains compatible.

Plausible accepts fixed Import Started, Data Import, Chart Edited, Project Loaded, Example Loaded and Chart Download events, alongside Source Download, SupaMakers Visit and AnimCharts Visit. Properties use bounded tool, method, control and format values only. No captions, filenames, data, row IDs, source links or raw query strings enter analytics. Opening design controls is not an edit; changed design state is measured on blur. Static download links measure clicks; client exports measure completed file preparation, not proof the user saved or published the file.

## Editorial template library — v0.12.0

The user approved 12 editable templates across three substantive collections: `/editorial-charts/dumbbell-examples/`, `/editorial-charts/slopegraph-examples/`, and `/editorial-charts/small-multiples-examples/`. They add three canonical resources (119 sitemap URLs / 120 site HTML pages), not twelve indexed template permutations. Research, weak exact-keyword evidence and attribution limits are recorded in `docs/EDITORIAL-TEMPLATES-2026-09-09.md`.

`src/lib/editorial-template-ids.ts` contains the public ID/kind whitelist; `src/data/editorial-templates.ts` owns the complete projects, interpretation and caveats. Three views use retained World Bank data; nine use clearly labelled original fictional CC0 data. Generated SVG/PNG/PDF/CSV/JSON assets under `public/editorial/templates/` remain out of Git/source ZIP and are rebuilt. Never relabel fictional data as findings.

`?template=<fixed-id>` loads only in the matching maker and canonicalizes to the maker URL. Input replacement retains style but clears unchanged sample semantics and old annotations; user-authored source/title/unit text is preserved. Initial publication mode with user data must not mark that custom text as replaceable sample text. Arbitrary restored projects preserve their own metadata and reset session template attribution.

Plausible Template Selected and Chart Download accept the fixed template property only when its kind matches the tool. Gallery links and explicit editor selections count selections; query loading alone does not duplicate the event. The template property persists through data replacement during the editor session, resets on another default example/project restore, and never derives from arbitrary filenames or URLs. It is not a cross-session or person-level attribution mechanism. Do not import template project data into the homepage analytics bundle; use the lightweight ID whitelist.
