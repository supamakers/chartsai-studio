...
import { dotPresets, radarPresets } from '../src/lib/presets.ts';
...
async function collect(directory) {
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (path === 'public/downloads') continue;
    if (entry.isDirectory()) await collect(path);
    else if (entry.isFile()) archive[`chartsai/${path}`] = new Uint8Array(await readFile(path));
  }
}
for (const directory of ['src', 'public', 'scripts', 'tests']) await collect(directory);
for (const path of [
  'README.md',
  'LICENSE',
...
console.log(
  `Prepared 6 original datasets, social image and source archive (${Object.keys(archive).length} files).`,
);
